...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
    ...
    // Attempt to find treatment from detail data first
    let treatment = getTreatmentDetail(slug)

    // If not found, look up by name in master list
    if (!treatment) {
        const name = MASTER_TREATMENTS.find(t => slugify(t) === slug)
        if (name) {
            return {
                title: `${name} | Treatment at Stork Hospital`,
                description: `Learn more about ${name} and related medical care at Stork Multispecialty Hospital Hyderabad.`
            }
        }
    }
...
export default async function TreatmentDetailPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    // 1. Try to get detailed data
    const detailedTreatment = getTreatmentDetail(slug)

    let treatment: any = detailedTreatment

    // 2. Fallback to master list name if detailed data is missing
    if (!treatment) {
        const name = MASTER_TREATMENTS.find(t => slugify(t) === slug)
        if (name) {
            treatment = {
        ...
        <div className="flex flex-col min-h-screen bg-white font-sans text-slate-900">
            ...
            <Section className="bg-gradient-to-b from-slate-50 to-white pt-32 pb-16 relative overflow-hidden">
                <div className="container max-w-6xl mx-auto px-6 relative z-10">
                    ...
                    {/* Breadcrumb - Updated to /treatments */}
